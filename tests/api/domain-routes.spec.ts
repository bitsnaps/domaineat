...
vi.mock('../../api/models/index.js', () => ({
  ...
  User: {},
  Domain: { findAll: vi.fn(async () => []), findByPk: vi.fn(async () => null), create: vi.fn() },
...
describe('API: Domain Validation (/api/validate)', () => {
  ...
  it('returns 400 for invalid domain format', async () => {
    const res = await app.request('/api/validate?domain=notadomain')
    expect(res.status).toBe(400)
    const data = await res.json()
  ...
  it('returns available=true when RDAP finds no record', async () => {
    ...
    const res = await app.request('/api/validate?domain=available123.com')
  ...
  it('returns available=false with whois data when domain is registered', async () => {
    ...
    const res = await app.request('/api/validate?domain=google.com')
    expect(res.status).toBe(200)
    const data = await res.json()
  ...
  it('strips www. prefix from domain', async () => {
    ...
    const res = await app.request('/api/validate?domain=www.example.com')
    expect(res.status).toBe(200)
    const data = await res.json()
...
describe('API: Domain Search (/api/search)', () => {
  ...
  it('returns extension results for a domain', async () => {
    ...
    const res = await app.request('/api/search?domain=example&tlds=com,org,net')
    expect(res.status).toBe(200)
    const data = await res.json()
    expect(data.status).toBe('ok')
  ...
  it('strips leading dots from TLDs', async () => {
    ...
    const res = await app.request('/api/search?domain=example&tlds=.io,.dev')
    expect(res.status).toBe(200)
