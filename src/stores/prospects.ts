...
export const useProspectsStore = defineStore('prospects', () => {
	...
	async function fetchProspects() {
		...
		try {
			...
			toast.error(`Failed to fetch prospects: ${msg}`)
		} finally {
			loading.value = false
		}
	}
	...
	async function createProspect(payload: ProspectCreate) {
		...
		try {
			...
			const prospect = res.data as Prospect
			...
			error.value = msg
			const toast = useToastStore()
			toast.error(`Failed to add prospect: ${msg}`)
			throw err
		} finally {
			loading.value = false
		}
	...
	async function updateProspect(id: number, payload: ProspectUpdate) {
		...
		error.value = null
		try {
			const res = await api.put(`/prospects/${id}`, payload)
			const updated = res.data as Prospect
			...
			error.value = msg
			const toast = useToastStore()
			toast.error(`Failed to update prospect: ${msg}`)
			throw err
		} finally {
			loading.value = false
		}
	...
	async function deleteProspect(id: number) {
		...
		try {
			...
			error.value = msg
			const toast = useToastStore()
			toast.error(`Failed to delete prospect: ${msg}`)
			throw err
		} finally {
			loading.value = false
		}
