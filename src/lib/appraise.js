...
function scoreLength(sld) {
    const len = sld.length;
    if (len <= 2)
        return { score: 10, label: `${len} chars (premium)`, passed: true };
    if (len <= 3)
        return { score: 9, label: `${len} chars (excellent)`, passed: true };
    if (len <= 4)
        return { score: 8, label: `${len} chars (very good)`, passed: true };
    if (len <= 5)
        return { score: 7, label: `${len} chars (good)`, passed: true };
    if (len <= 6)
        return { score: 6, label: `${len} chars (fair)`, passed: true };
    if (len <= 8)
        return { score: 5, label: `${len} chars (average)`, passed: false };
    if (len <= 10)
        return { score: 4, label: `${len} chars (below avg)`, passed: false };
    if (len <= 12)
        return { score: 3, label: `${len} chars (long)`, passed: false };
    return { score: 1, label: `${len} chars (very long)`, passed: false };
}
/** Score based on TLD prestige. */
function scoreTld(tld) {
...
function splitIntoKnownWords(s) {
    const result = [];
    let remaining = s;
...
function scoreBrandable(sld) {
    ...
    if (len >= 3 && vowelRatio >= 0.25 && vowelRatio <= 0.7) {
        if (len <= 5)
            return { score: 8, label: 'Short and pronounceable', passed: true };
        if (len <= 8)
            return { score: 7, label: 'Pronounceable', passed: true };
...
function scoreClean(sld) {
    const hasHyphen = sld.includes('-');
    const hasNumber = /\d/.test(sld);
    const hasSpecial = /[^a-z0-9-]/i.test(sld);
    if (hasSpecial)
        return { score: 1, label: 'Contains special characters', passed: false };
    if (hasHyphen && hasNumber)
        return { score: 1, label: 'Has hyphen AND number', passed: false };
    if (hasHyphen)
        return { score: 3, label: 'Contains hyphen', passed: false };
    if (hasNumber)
        return { score: 3, label: 'Contains number', passed: false };
...
export function gradeToRange(grade) {
    const ranges = {
 ...
 */
export function appraise(domain) {
